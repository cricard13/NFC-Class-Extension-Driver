...
static NFCSTATUS phNciNfc_Init(void *pContext)
{
    ...
    if ((PHNCINFC_GETNCICONTEXT()->ResetInfo.NciVer & PH_NCINFC_VERSION_MAJOR_MASK) ==
        (PH_NCI2xNFC_VERSION & PH_NCINFC_VERSION_MAJOR_MASK))
    {
        TxInfo.Buff = (uint8_t *)phOsalNfc_GetMemory((uint32_t)2);
        TxInfo.wLen = 2;
...
static NFCSTATUS phNciNfc_ProcessInitRsp(void *pContext, NFCSTATUS Status)
{
    ...
    if (NULL != pNciContext)
    {
        ...
        else if ((pNciContext->ResetInfo.NciVer & PH_NCINFC_VERSION_MAJOR_MASK) ==
            (PH_NCI2xNFC_VERSION & PH_NCINFC_VERSION_MAJOR_MASK))
        ...
        if (pNciContext->RspBuffInfo.wLen >= 19)
        {
            if (pNciContext->RspBuffInfo.pBuff[Offset++] == PH_NCINFC_STATUS_OK)
            {
                ...
                if (pInitRsp->NoOfRfIfSuprt <= PH_NCINFC_CORE_MAX_SUP_RF_INTFS)
                {
                    ...
                    if (pInitRsp->DataHCIPktPayloadLen > 0)
                    {
...
static void phNciNfc_DelayForCreditNtfCb(void* pContext, uint8_t bCredits, NFCSTATUS status)
{
    ...
    {
        if (NULL != pNciContext &&
            ((pNciContext->ResetInfo.NciVer & PH_NCINFC_VERSION_MAJOR_MASK) ==
            (PH_NCI2xNFC_VERSION & PH_NCINFC_VERSION_MAJOR_MASK)))
        {
